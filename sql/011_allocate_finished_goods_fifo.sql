-- Allocate Finished Goods FIFO (DEV-022)
-- Run in Supabase SQL editor after sql/010_finished_goods_batch_consumptions.sql.
--
-- Atomically allocates finished-goods quantity from immutable production_batches
-- using FIFO (oldest produced_at, then batch id) and appends immutable
-- finished_goods_batch_consumptions ledger rows (direction = out).
--
-- Remaining quantity is NEVER stored. Availability is always:
--   Remaining = produced_quantity − SUM(out) + SUM(in)
--
-- Does not modify production_batches or Production RPCs.
-- Does not implement services, hooks, or UI.
...
BEGIN
  ...
  -- Reject duplicate posting for the same source document line.
  IF EXISTS (
    SELECT 1
...
COMMENT ON FUNCTION allocate_finished_goods_fifo(
  ...
  'FIFO allocate finished goods and append immutable batch consumption ledger rows. Remaining is calculated only.';
